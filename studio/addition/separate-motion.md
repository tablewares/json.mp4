Good catch — the previous version only let rotation ride along as an instant `rotateDeg` or as a decaying offset baked into the in/out phases. Two changes below:

1. **Temporal sequencing**: rotation now defaults to starting only *after* the entrance move/fade has finished (`startAt: "afterIn"`) — so by the time it starts spinning, `translateX/Y` from the `in` phase are already back to 0, and the transform is effectively pure rotation around the asset's own center. That's what "resolve rotation after moving into the scene" means mechanically — no change to the CSS transform order was needed for this, since translate is already applied as a fixed screen-space vector regardless of rotation (see note at the end if you want the reasoning).
2. **Animated rotation**: new `motion.rotate` phase — `fromDeg → toDeg` over `durationInFrames`, with its own easing, layered as the *base* rotation that the transient in/out offsets still add on top of.

## `src/motion/motion.js` (full replacement)
...
Extend the `motionSpec` definition added last turn and add `motionRotatePhase`:
...
(`motionInPhase`/`motionOutPhase` from last turn are unchanged.)

## No changes needed elsewhere

- **`resolveScene.js`**: already calls `resolveMotion(assetSpec.motion)` — the richer return shape flows through untouched.
- **`Composition.jsx`**: already calls `computeMotionTransform(asset.resolvedMotion, frame, asset.timing)` and applies `translate(...) rotate(...)`. That order is unaffected — translate is always a fixed screen-space vector applied after rotation in CSS's matrix composition, so it never "orbits" regardless of rotation angle; the sequencing fix above is what makes rotation visually happen after the move, not a transform-order change.
- **`ProjectBuilder.js`**'s `checkMotionSpec`/`checkMotionAliases` (added last turn) need no changes — they validate the whole `motionSpec` against the schema and dry-run `resolveMotion`, both of which now automatically cover `rotate`.
- **CLI** (`--motion '{"rotate":{"toDeg":20}}'`) needs no changes — it already passes the raw JSON blob through.

Example:

```json
{ "in": "fadeUp", "rotate": { "toDeg": 8, "durationInFrames": 24, "easing": "easeOut" } }
```

→ slides up while fading in, then once settled, eases into an 8° tilt over the next 24 frames.